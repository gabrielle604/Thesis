...
svyboxplot(~log(monoEthyl)~factor(gender), nhc, all.outliers=TRUE, xlab = "Gender", main = "Bivariate Analysis of (Logged) Urinary Mono-ethyl Phthalate and Gender")

svyboxplot(~log(monoEthyl)~factor(gender), subset_child, xlab = "Gender", main = "Bivariate Analysis of (Logged) Urinary Mono-ethyl Phthalate and Gender")

svyboxplot(~log(monoEthyl)~factor(gender), subset_adult, xlab = "Gender", main = "Bivariate Analysis of (Logged) Urinary Mono-ethyl Phthalate and Gender")

```


## Boxplots, bivariate analyses (combined children and adults)
```{r}
...
## ref ED
noNAs = NHANES %>% filter(!is.na(refED)) %>% filter(!is.na(monoEthyl))

box_education <- ggplot(data = noNAs, design=nhc,
                      aes(x=log(monoEthyl), y=refED, fill=refED)) +
  scale_fill_brewer(palette="PuBuGn") +
  geom_boxplot() +
  theme(text = element_text(size=12)) +
  xlab("(logged) Mono-Ethyl Phthalate Level (ng/mL)") +
  ylab("Household reference person education level") +
ggtitle("Household reference person education level and Logged Phthalate Level")


box_education
```
## Bivariate for children:
```{r}
# change the noNAs dataset with each boxplot I create:
...
box_age <- ggplot(data = noNAs, design=nhc,
...
box_refED <- ggplot(data = noNAs, design=nhc,
                      aes(x=log(monoEthyl), y=refED, fill=refED)) +
  scale_fill_brewer(palette="PuBuGn") +
  geom_boxplot() +
  theme(text = element_text(size=12)) +
...
box_gender <- ggplot(data = noNAs, design=nhc,
...
box_eth <- ggplot(data = noNAs, design=nhc,
...
box_fpl <- ggplot(data = noNAs, design=nhc,
...
box_cit <- ggplot(data = noNAs, design=nhc,
                      aes(x=log(monoEthyl), y=citizenship, fill=citizenship)) +
  scale_fill_brewer(palette="PuBuGn") +
  geom_boxplot() +
  theme(text = element_text(size=12)) +
...
```{r}
svyboxplot(~log(monoEthyl)~factor(year), nhc, all.outliers=TRUE)
```
...
## AGE
noNAs = NHANES %>% filter(!is.na(age)) %>% filter(!is.na(monoEthyl))

Age <- ordered(noNAs$age, levels = c("child", "young adult", "middle-aged", "older adult"))

box_age <- ggplot(data = noNAs, design=nhc,
                      aes(x=log(monoEthyl), y=Age, fill=Age)) +
  scale_fill_brewer(palette="PuBuGn") +
  geom_boxplot() +
  theme(text = element_text(size=12)) +
  xlab("(logged) Mono-Ethyl Phthalate Level (ng/mL)") +
...
box_year <- ggplot(data = noNAs, design=nhc,
  ...
  xlab("(logged) Mono-Ethyl Phthalate Level (ng/mL)") +
  ylab("Year of Survey") +
ggtitle("Year of Survey and Logged Phthalate Level")
...
box_refED <- ggplot(data = noNAs, design=nhc,
  ...
  xlab("(logged) Mono-Ethyl Phthalate Level (ng/mL)") +
  ylab("Household Reference Person's Education Level") +
ggtitle("Ref. Person's Edu. Level and Logged Phthalate Level")
...
# regTermTest(subset_child,~refED)
```
...
## learning how to make factors and re-leveling
...
subset_adult$ethnicity <- as.factor(subset_adult$ethnicity)
is.factor(subset_adult$ethnicity)

subset_adult$ethnicity <- relevel(subset_adult$ethnicity, ref = "Non-Hispanic White")


```


## trying to quantify the missing data:
Only 1.4% of the data is missing among the children subset and the adult subset.

```{r}
for_children<-c('year','WTINT2YR','SDMVPSU','SDMVSTRA','DMDHREDU','RIDAGEYR','RIAGENDR','RIDRETH1','INDFMPIR','DMDCITZN','URXMEP')
for_adults<-c('year','WTINT2YR','SDMVPSU','SDMVSTRA','DMDEDUC2','RIDAGEYR','RIAGENDR','RIDRETH1','INDFMPIR','DMDCITZN','URXMEP')
children_find<-subset(only_child,select=for_children)
adults_find<-subset(only_adults,select=for_adults)
```

```{r}
vis_miss(children_find, sort_miss = TRUE)
```

```{r}
vis_miss(adults_find, sort_miss = TRUE)
```
