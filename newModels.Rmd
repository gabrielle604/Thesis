...
```{r}
wil <- svyranktest(log(monoEthyl)~gender, design = nhc, na = TRUE, test = c("wilcoxon"))
wil
```
...
```{r}
mtest <- svyranktest(log(monoEthyl)~gender, design = nhc, na = TRUE, test=("median"))
mtest
...
logit2 <- (svyglm(as.factor(DMDCITZN)~as.factor(DMDHREDU)+RIDAGEYR, family=quasibinomial, design=subset1, na.action = na.omit))
summary(logit2)
```
